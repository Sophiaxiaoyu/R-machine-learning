...
diamonds %>%filter(cut == "Ideal",clarity == "IF")%>%
  ggplot(mapping = aes(x = color , y = price)) +
  geom_boxplot(
    mapping = aes(fill = carat >median(diamonds$carat)))+
  facet_grid(clarity~cut)+
  theme(legend.position = "top")
```
...
ggplot(data=mpg,mapping = aes(x=displ,y=hwy))+
  geom_point()+
  geom_smooth(mapping = aes(color= drv),method = lm,formula = y ~ x)
```
...
ggplot(data=mpg,mapping = aes(x=displ,y=hwy))+
  geom_point()+
  geom_smooth(mapping = aes(group= drv),method = lm,formula = y ~ x)
```
...
$$
\log\left(x^2\right)+\log\left(y^2\right)
$$
$$
\log w = z
$$
