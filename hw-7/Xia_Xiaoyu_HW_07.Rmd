...
```{r}
 tibble::rowid_to_column(quad_data_big) %>% 
...
There are 5 rows and 3 columns in small data design matrix.

There are 30 rows and 3 columns in big data design matrix.

The Intercept is contained in the FIRST column, it shows the value of y when x equals to 0.
...
```
There are 3 rows and 3 columns in EACH posterior covariance matrix.
...
#### SOLUTION
```{r}
diag(covmat_quad_small_noprior) %>% sqrt()
```

```{r}
diag(covmat_quad_big_noprior) %>% sqrt()
```
...
```
The values won't change if $\sigma$ was assumed to equal 5 instead of 1, because in the process of our calculation, there is no parameter related to the value of sigma.
...
```{r}
tibble::tibble(
coeff = colnames(Xmat_quad_small),
...
```
What do you think?  
The quadratic feature considered to be statistically significant in the both case.
In the small data case,The interval values are all positive values.
In the big data case,The interval values are all negative values.
...
B0_inform <- 1.4^2 *  ncol(Xmat_quad_small)%>%diag()
B0_inform
```
...
B0_vague <- 30^2 * ncol(Xmat_quad_small)%>%diag()
B0_vague
```
...
#### SOLUTION
```{r}
diag(covmat_quad_small_inform) %>% sqrt()
```
```{r}
diag(covmat_quad_big_inform) %>% sqrt()
```

In the small data case,the posterior standard deviation on the quadratic feature based on the INFORMATIVE prior is different from the result from 1f.
In the big data case,the posterior standard deviation on the quadratic feature based on the INFORMATIVE prior is similar to the result from 1f.
...
#### SOLUTION
```{r}
diag(covmat_quad_small_vague) %>% sqrt()
```
```{r}
diag(covmat_quad_big_vague) %>% sqrt()
```
Both posterior standard deviations in two datasets are similar to the result from 1f.
...
What do you think? 
Using an informative prior in regression analysis can restrict the possible values of the coefficients and decrease the level of uncertainty in the estimates. Conversely, a vague prior doesn't provide much restriction on the coefficients. The level of uncertainty in the estimates is influenced by the a priori standard deviation, with smaller values leading to smaller uncertainty intervals. When working with small data sets, setting meaningful prior intervals can help decrease posterior uncertainty. However, with large data sets, the influence of the prior becomes negligible as the data overwhelms it.
...
```
```{r}
diag(laplace_quad$var_matrix) %>% sqrt()
```
...
```
The probability that the slope is positive is 0.
...
30 rows: 30 observations.
2500 columns:2500 posterior samples.
...
mae_quad_post_03
```
The three MAE values are not the same，because all parameters about $\beta$ have different values.
...
Because the value of $\beta$ effects the MAE.
...
There are 150 rows in the training design matrix and 50 rows in the test matrix.
...
I think the degree of coincidence is the highest when the degree is between 1-10, so it is a bit overfitting.
...
As we can see, Model which gets 7 degrees of freedom is the best.
...
The model performance on the training set x training results are that the more complex the model the better the results without overfitting.
However, we compare based on the test set, the model with a degree of freedom of 7 performs the best.

The difference in conclusions between the performance of the model on the training set versus the test set may be due to overfitting. Overfitting occurs when a model is too complex and is fitted too closely to the training data, such that it fails to generalize well to new, unseen data.

In this case, it is possible that the more complex model with a higher degree of freedom performed better on the training set due to overfitting, while the model with a degree of freedom of 7 performed better on the test set because it was able to generalize better to new data.

Therefore, it is important to evaluate a model's performance on both the training and test sets to determine if it is overfitting and whether it can generalize well to new data. This can help ensure that the conclusions drawn from the model are reliable and accurate.
