...
#### SOLUTION
The formula for a Gaussian prior on the unknown mean is showed below:
$$
\mu_{0}-2\tau_{0}=10
$$
$$
\mu_{0}+2\tau_{0}=12
$$
Solving a system of linear equations in two variables gives：
$$
\mu_{0}=11
$$
...
param_grid %>% 
  ...
  theme_bw()
```
The center of the smallest black cycle is the posterior mode.
The orange node represents the sample average and standard deviation.
As we can see, the sample average is smaller than the $\mu$ parameter. The are not similar.The standard deviation is similar to the $\sigma$ parameter.

The posterior uncertainty in $\mu$ based on the visualization shows a narrow posterior distribution, it suggests that there is relatively low uncertainty in the estimates of $\mu$. Conversely, a wider $\sigma$ posterior distribution suggests higher uncertainty in the estimates.
...
#### SOLUTION
$$
p\left(\mu \mid \mathbf{x}, \sigma\right) \propto \prod_{n=1}^{N} \left( exp \left( \frac{-1}{2{\sigma}^2}\sum (x_n-\mu)^2 \right) \right) \times exp\left( \frac{-1}{2\tau_0^2} (\mu-\mu_0)^2\right)
$$
Apply the natural log:
$$
log\left(p\left(\mu \mid \mathbf{x}, \sigma\right)\right) \propto \frac{-1}{2{\sigma}^2} \sum_{n=1}^{N} \left((x_n-\mu)^2\right) -\frac{1}{2\tau_0^2} (\mu-\mu_0)^2
$$
$$
\frac{d}{d\mu} \left( \log \left( p\left( \mu \mid \mathbf{x}, \sigma \right) \right) \right) = -\frac{1}{2\sigma^2} \sum_{n=1}^{N} \left( \frac{d}{d\mu}\left( \left( x_n - \mu \right)^2 \right) \right) - \frac{1}{2\tau_{0}^2} \frac{d}{d\mu}\left( \left( \mu - \mu_0 \right)^2 \right)
$$
simplify the formula respectively:
$$
-\frac{1}{2\sigma^2} \sum_{n=1}^{N} \left( \frac{d}{d\mu}\left( \left( x_n - \mu \right)^2 \right) \right) = \frac{1}{\sigma^2} \sum_{n=1}^{N} \left( x_n - \mu \right) 
$$
$$
\frac{1}{\sigma^2} \sum_{n=1}^{N} \left( x_n - \mu \right) = \frac{1}{\sigma^2} \left( \sum_{n=1}^{N}x_n - \sum_{n=1}^{N}\mu \right)  = \frac{N}{\sigma^2}\left( \bar x - \mu \right)
$$
$$
- \frac{1}{2\tau_{0}^2} \frac{d}{d\mu}\left( \left( \mu - \mu_0 \right)^2 \right) = - \frac{1}{\tau_0^2} \left( \mu - \mu_0 \right)
$$
$$
\frac{d}{d\mu} \left( \log \left( p\left( \mu \mid \mathbf{x}, \sigma \right) \right) \right) = \frac{N}{\sigma^2}\left( x_n - \mu \right) - \frac{1}{\tau_0^2} \left( \mu - \mu_0 \right)
$$
Derived again:

$$
\frac{d}{d\mu} \left(\frac{d}{d\mu} \left( \log \left( p\left( \mu \mid \mathbf{x}, \sigma \right) \right) \right)\right) = \frac{d}{d\mu} \left(\frac{N}{\sigma^2}\left( x_n - \mu \right)\right) - \frac{d}{d\mu} \left(\frac{1}{\tau_0^2} \left( \mu - \mu_0 \right)\right)
$$
$$
\frac{d}{d\mu} \left(\frac{d}{d\mu} \left( \log \left( p\left( \mu \mid \mathbf{x}, \sigma \right) \right) \right)\right) =-\frac{N}{\sigma^2}-\frac{1}{\tau_0^2}
$$

Thus:
$$
\frac{d^2}{d\mu^2}\left( \log \left( p\left( \mu \mid \mathbf{x}, \sigma \right) \right) \right) =-\frac{N}{\sigma^2}-\frac{1}{\tau_0^2}
$$
...
What do you think?  
The  sign of the second derivative of the log-posterior can help us judge the opening direction of the function. If the second derivative is positive, the function opens upwards and the function has a minimum. The opposite is true for negative numbers.
So we can confirm that the mode does in fact correspond to the $\mu$ value associated with the maximum log-posterior density.
...
#### SOLUTION
According to the expression for the Gaussian distribution：
$$ 
p\left( \mu \mid \mathbf{x}, \sigma \right) \approx N \left( \mu \mid \mu , \sigma^ 2 \right)
$$
The parameter value that maximizes the posterior is called the MAP , or max a posteriori estimate.
So the expressions for the approximate posterior mean is the same as the posterior mode:
$$
\mathrm{m}_N= \mu_{MAP}
$$

After performing the Laplace Approximation we can visualize:
$$
P\left(\theta \mid x  \right) \approx N\left(\hat \theta,[-H\mid_{\theta=\hat\theta}]^{-1} \right)
$$

$$
H=\frac{d^2}{d\mu^2}\left( \log \left( p\left( \mu \mid \mathbf{x}, \sigma \right) \right) \right) =-\frac{N}{\sigma^2}-\frac{1}{\tau_0^2}
$$

compare with the previous formulas：
$$
\mathrm{s}_N^2=\sigma^2=[-H\mid_{\theta=\hat\theta}]^{-1}
$$
The expression for posterior standard deviation:
$$
\mathrm{s}_N^2=-\left(-\frac{N}{\sigma^2}-\frac{1}{\tau_0^2}\right)^{-1}=\left(\frac{N}{\sigma^2}+\frac{1}{\tau_0^2}\right)^{-1}
$$
### 2d)
...
What do you think?  
The Laplace approximation is equal to the exact posterior distribution for the one parameter normal-normal model with an unknown mean because the posterior distribution in this case is a normal distribution. 
The Laplace approximation gives the exact mean and variance of the distribution by finding the mode of the log-posterior, which is the same as the posterior mean, and using the Hessian of the log-posterior at the mode to compute the posterior precision, which is the reciprocal of the variance of the posterior distribution. 
Therefore, for this specific application, the Laplace approximation is not an approximation, but an exact solution.
...
What do you think?  
Transforming $\sigma$ to $\varphi$ using a transformation like the natural log when performing inference with the Laplace approximation is useful because it allows the posterior distribution to be more easily approximated by a normal distribution, which is a key assumption of the Laplace approximation. By transforming $\sigma$ to $\varphi$, the posterior distribution can be made more symmetric, which makes it more amenable to approximation by a normal distribution. Additionally, this transformation can help prevent issues such as negative values for the standard deviation, which can be problematic in certain models.
...
#### SOLUTION
$$ 
p \left( \mu, \varphi \mid \mathbf{x} \right) \propto \prod_{n=1}^{N} \left( \mathrm{normal} \left(x_n \mid \mu, g^{-1} \left( \varphi \right) \right) \right) \times \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right) \times \mathrm{Exp}\left(g^{-1} \left( \varphi \right) \mid \lambda \right)\times|\frac{\varphi}{d_\varphi} \left( g^{-1} \left( \varphi \right)\right)|
$$
...
$$
\sigma= g\left( \varphi  \right)^{-1} = Exp \left( \varphi  \right)
$$

Write the log of the derivative adjustment in an equation block.  
$$
log\left(d\left(\sigma\right)\right)=log\left(\frac{d}{d\varphi} |g\left( \varphi  \right)^{-1}|\right)=log\left(Exp \left( \varphi  \right)\right)=\varphi
$$
...
map_res_02
```
The identified optimal parameter values are the same. The Hessian matrices are the same. The only different between the optimizations is the function gradient.
The log-posterior surface shows a hint about there is only one optimum, so there is no difference on performance even though the function gradient is different.
...
Display the posterior means on each parameter:
```{r}
mean1<-laplace_result$mode
mean1
```
Display the posterior standard deviations on each parameter.
```{r}
sd1<-sqrt(diag(laplace_result$var_matrix))
sd1
```
 the posterior correlation coefficient between $\mu$ and $\varphi$:
```{r}
coefficient1<-laplace_result$var_matrix[1, 2] / 
  (sqrt(laplace_result$var_matrix[1,1]) * sqrt(laplace_result$var_matrix[2,2]))
coefficient1
```
 The posterior correlation coefficient between $\mu$ and $\varphi$ is 0.1211003.
...
posterior_mean2
```
The result doesn't equal to the posterior mean on `sigma`.
...
```{r}
  ...
  geom_histogram(bins = 55) 
```
The posterior distribution on $\sigma$ look similarly Gaussian.
