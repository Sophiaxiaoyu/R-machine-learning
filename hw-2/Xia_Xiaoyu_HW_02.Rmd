...
What do you think?  
Model9 is the best according to the training set performance metrics.
Because the RMSE of modle9 is the lowest among all the models.
The two performance metrics agree on the best model.
...
What do you think?  
The coefficient estimates are dissimilar for those coefficients present in at least 2 models visualized in the figure above.

The level or amount of uncertainty doesn't associate with the coefficients consistent across the three models.
...
For model 9, when the value of x is between -2 and 1.5, the value of y fluctuates around 0, and the curve is smooth; when the value of x is greater than 1.5, the curve shows an exponential increasing trend.

It doesn't seem like the trends consistent with the training set.
...
What do you think?  
Model2 is the best according to the hold-out evaluation.
It's not the same model as identified from the training set performance.
The hold-out set rank shows the model 9 has the worst performance according to the RMSE.
...
We can't say something absolutely because all the confidence intervals cross the 0 value. But we can still say the fit_lm_9 is better, as we can see from the picture, the confidence interval for fit_lm_9 in each coefficient is much more smaller.
...
As we can seen from the picture, the fit_lm_2 is better.
When the confidence intervals cross the 0 value, it's nonsense for this coefficient. All the coefficients for the fit_lm_2 don't cross
0.
...
An individual model be trained 4 times and tested only once.
...
A:RMSE is a typical indicator of the regression model, which is used to indicate how much error the model will produce in the prediction. For larger errors, the weight is higher. y is the actual value, and y~ is the predicted value, the smaller the RMSE, the better.
...
A:The coefficient estimates are dissimilar for those coefficients present in at least 2 models visualized in the figure above.
...
A：Compared with the 1a problem, the visualization of this model is more fit for each point and smooth, and the RMSE of each point and the linear fitting value is smaller.
...
Which model is the best?  
A：Model 6 is the best one according to the RMSE.
...
What do you think?  
A:The resampling scheme does not identify the same model as the best model for the two noise cases.
The models identified to be poor for the high noise case remain poor for the low noise case.
...
Which model is the best?  
A:Model5 is the best one according to the RMSE.
...
What do you think?  
A:
The resampling scheme does not identify the same model as the best model for the two sample size cases.
The models identified to be poor for the low sample size case remain poor for the high sample size case.
...
What do you think?  
A:The larger the sample data and the smaller the noise level, the higher the ability to identify complex behaviors.
