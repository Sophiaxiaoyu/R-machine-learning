...
This dataset can be seen as a balanced dataset.
### 1c)
...
`ifelse(<conditional statement to check>, <value if TRUE>, <value if TRUE>)`  

Thus, the user must specify a condition to check as the first argument to the `ifelse()` function. The second argument is the value to return if the conditional statement is TRUE, and the second argument is the value to return if the conditional statement is TRUE.  
...
What do you think?  
Not really,there are several noisy points. For example, when x is near to -1.5,the observation is equal to 0,but the prediction is near to 1 with 90% probability. 
As for the majority of the dataset, when more data sets are concentrated on one side, the corresponding prediction curve also corresponds to a higher probability.                       
...
your response here.  
6 observations are associated with TRUE-Positives. 53 observations are associated with True-Negatives.
### 2d)

**You will now calculate the Sensitivity and TRUE Positive Rate (FPR) associated with the model predicted classifications based on a threshold of 0.5. This question is left open ended. It is your choice as to how you calculate the Sensitivity and FPR. However, you CANNOT use an existing function from a library which performs the calculations automatically for you. You are permitted to use `dplyr` data manipulation functions. Include as many code chunks as you feel are necessary.**  
...
**Are the levels of the binary outcome, `y`, balancednon_event** 
...
```
The levels of the binary outcome `y`  can be seen as balanced.
...
According to the Accuracy, model 3 is the best.
However, Accuracy is just one of the many factors that determine the quality of a machine learning model. Other factors such as interpretability, robustness, training time, generalization ability, and model complexity should also be taken into consideration when choosing the best model for a particular problem. Therefore, the answer to whether Model 3 is the best overall depends on the specific use case, context, and desired trade-offs.
...
Top two according to Accuracy is model 3 and model 2, and worst two models ars model 8 and model 5.

**How do the False-Positive and False-Negative behavior compare between these four models?**  

A lower False-Positive rate indicates that the model is less likely to predict an event (e.g. positive test result) when the true class is a non_event (e.g. negative test result). Similarly, a lower False-Negative rate indicates that the model is less likely to predict a non_event when the true class is an event.

As we can see, the False-Positive and False-Negative rates for model 3 are 7.7 and 11.6.
The False-Positive and False-Negative rates for model 2 are 9.0 and 12.3.
The False-Positive and False-Negative rates for model 8 are 17.4 and 28.4.
The False-Positive and False-Negative rates for model 5 are 21.9 and 19.4.

So, if a particular model has both a lower False-Positive rate and a lower False-Negative rate compared to another model, it would suggest that the former model is performing better in terms of both precision and recall.

Also, the False-Positive and False-Negative rates can be used to calculate the overall error rate of a classification model. The lower the error rate, the better the model is at accurately classifying events and non_events.

So we can say model 3 is the best model because of the lowest False-Positive and False-Negative rates.
...
Which model is the best?  
According to the ROC AUC, Model 3 is the best.
However, ROC AUC is just one of the many factors that determine the quality of a machine learning model. Other factors such as interpretability, robustness, training time, generalization ability, and model complexity should also be taken into consideration when choosing the best model for a particular problem. Therefore, the answer to whether Model 3 is the best overall depends on the specific use case, context, and desired trade-offs.
...
dotplot(roc_results)
```
Sensitivity=TP/(TP+FN), so the higher rate for Sensitivity, the higher True-Positive Rate for model.
Model 3 has the highest True-Positive Rate at the default threshold.
Specificity=TN/(TN+FP),so the higher rate for Specificity, the lower False-Positive Rate for model.
Model 3 has the lowest False-Positive Rate at the default threshold.
...
when the `threshold` is  low, Only in the 0-threshold interval, the value is 0. According to the observation of the scatter diagram, there is basically no value, so both FN and TN are basically 0.
So the `sensitivity` have values at or near 1 when the `threshold` is so low.
...
What do you think?  
Model 3 is the best according to the sensitivity.The result consistents with the ROC AUC summary metrics I calculated previously. 

We know that sensitivity represents the proportion of positive instances correctly identified as positive, while 1-specificity represents the proportion of negative instances incorrectly classified as positive.

A model with low sensitivity and high 1-specificity can be considered a completely invalid model, meaning that it will miss most positive instances and classify most negative instances as positive.

If we want to determine how close a model is to a completely invalid model, we can calculate the average of sensitivity and 1-specificity. The closer the fitted curve is to the 45-degree line, the closer the model is to a completely invalid model.

Therefore, model 8 is the closest to the "completely invalid model".
...
What do you think?  
The values in the `event` column and the `non_event` column represent for the probability of whether this event is `event` or `non_event`.
So the sum of this two values is 1.
...
As shown in the lecture slides, we can create a stacked bar chart to get a rough idea about the number of events and non_events within each predicted probability bin. This is simple to do with `ggplot2` by using the `fill` aesthetic associated with the `geom_bar()` geometric object.  
...
**Do any bins consist of only events? Do any bins consist of only non_events?**  
...
According to the plot,bin [0.9,1] consist of only events. No bins consist of only non_events.  

### 6g)

Instead of showing the counts within each bin, let's change the bar chart so the maximum height is 1. The stacked bar chart will therefore show the proportion of events and non_events within each predicted probability bin.  
...
What do you think?  
The empirical proptions of the event doesn't consistent with the predicted probability for model 1.
...
What do you think?  
The calibration curve doesn't consistent with my stacked filled barchart created previously.
...
What do you think?  
A well-calibrated model is one that accurately represents the confidence of its predictions.

The most obvious aspect of observation to judge the performance of a calibration curve is the degree of calibration. The visual inspection chart is used to assess whether the calibration curve is close to the 45 degree line. If so, the model is well calibrated. If there are data points that are off, the model is either too confident or not confident enough in its predictions.

As we can see from these there plots, it's hard to tell whether model 1 or model 3 is the best. If i have to choose one，model 1 appears the most well calibrated.
...
```
The created calibration curve consistent with my manually created curve from the previous problems.
...
```
The created calibration curve consistents with my manually created curve from the previous problems.
...
```
The created calibration curve doesn't consistent with my manually created curve from the previous problems.
