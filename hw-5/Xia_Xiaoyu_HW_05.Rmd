...
The prior $\pm 2$ standard deviation interval or roughly 95% uncertainty interval around the prior mean, $\mu_{0}$, is between 115 units and 135 units.

So we can get these two formula:
$$
\mu_{0}-2\tau_{0}=115
$$
$$
...
percentile_75th
```
The interval between these two quantiles contains 50% of the probabilty mass.
...
$$
Based on $\sigma = 5$ and your result in Problem 1a), how many observations are required to have a posterior standard deviation on $\mu$ to be $\tau_{N} = 2$?
$$
\sigma = 5 ;
\tau_{N} = 2 ;
\tau_{0} = 5
$$
Based on these known conditions we derive：
So in this situation, we need 6 observations to have a posterior standard deviation.
```{r}
N1<-5^2 * ((1/2^2) - (1/5^2))
N1
```

How many observations are required to have $\tau_{N} = 0.5$?
$$
\sigma = 5 ;
\tau_{N} = 0.5 ;
\tau_{0} = 5
$$
Based on these known conditions we derive：
So in this situation, we need 99 observations to have a posterior standard deviation.
```{r}
...
N3
```
So in this situation, we need 25 observations.
...
$$
It is obvious that the answer is:
$$
...
The expression we get is the same as the mean $\mu_{N}$ for posterior.
The reason is that the two expressions are belonged to Gaussian distribution and the two things, mode and mean, are the same thing in Gaussian distribution.
...
$$
p \left(\sigma \mid \mathbf{x}, \mu\right) \propto \prod_{n=1}^{N} \left( \frac{1}{\sqrt{2\pi\sigma^2}} \exp \left( {-\frac{1}{2} \left( \frac{x-\mu}{\sigma} \right)^2 } \right)  \right)  \times \lambda \exp \left(-\lambda \sigma \right)
$$
$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto 
\sum_{n=1}^{N} \left( \log \left( \frac{1}{\sqrt{2\pi\sigma^2}} \exp \left( - {\frac{1}{2} \left( \frac{x-\mu}{\sigma} \right)^2 } \right) \right)  \right)
 + \log \left( \lambda \exp \left(-\lambda \sigma \right) \right)
$$
Due to the algorithm of log, it is known that
$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto
-\frac{1}{2} \sum_{n=1}^{N} \left( \log \left( \sigma^2 \right) \right)- \frac{1}{2\sigma^2} \sum_{n=1}^{N} \left( \left( x_{n}-\mu \right)^2 \right) - \lambda\sigma
$$
$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto
-\frac{N}{2}\log \left( \sigma^2 \right) -\frac{1}{2\sigma^2} \sum_{n=1}^{N} \left( \left( x_{n}-\mu \right)^2 \right) - \lambda\sigma
$$
### 3b)
...
$$
because of the answer of previous problem 3a), it is easy to get
$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto
-\frac{N}{2}\log \left( \sigma^2 \right) -\frac{Nv}{2\sigma^2} - \lambda\sigma
$$
The quantity $v$ is referred to as being "sufficient", because we can know that $v$ has the whole information related to calculate the $\mu$ and $\sigma$ and so on from the formula. After making use of $v$, the expression for the un-normalized log-posterior does not depend on the individual observations.
### 3c)
...
What do you think?  
The log-posterior has one mode and the shape of the log-posterior does not look like a parabola.
### 3e)
...
$$
$$
\frac{d}{d\varphi} \exp \left( \varphi \right) = \exp \left( \varphi \right)
$$
$$
...
What do you think?  
Although the shape is not a complete prabola, the log-posterior looks like more parabolic now.
## Problem 04
