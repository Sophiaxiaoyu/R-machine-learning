...
What do you think?  
The answer for this two problem is the same.
...
$$
p(x\mid\mu)= \prod\limits_{n=1}\limits^{N}({\mu^{x_n}(1-\mu)^{1-x_n}})=\prod\limits_{n=1}\limits^{N}(Bnoulli(x_n|\mu))
$$
$$
log[p(x\mid\mu)]=log\left(\prod\limits_{n=1}\limits^{N}({\mu^{x_n}(1-\mu)^{1-x_n}}) \right)
$$
$$
log[p(x\mid\mu)]=\sum\limits_{n=1}\limits^{N}\left(log[{\mu^{x_n}(1-\mu)^{1-x_n}}] \right)
$$
$$
log[p(x\mid\mu)]=\sum\limits_{n=1}\limits^{N}\left(log[{\mu^{x_n}]+log[(1-\mu)^{1-x_n}}] \right)
$$
$$
log[p(x\mid\mu)]=\sum\limits_{n=1}\limits^{N}\left({x_nlog[\mu]}+(1-x_n)log[1-\mu] \right)
$$
$$
log[p(x\mid\mu)]=\sum\limits_{n=1}\limits^{N}\left({x_nlog[\mu]}\right)+\sum\limits_{n=1}\limits^{N}\left((1-x_n)log[1-\mu]\right)
$$
$$
log[p(x\mid\mu)]=log[\mu]\sum\limits_{n=1}\limits^{N}\{x_n \}+log[1-\mu]\sum\limits_{n=1}\limits^{N}\{1-x_n\}
$$
...
value_5
```
The length of the returned result is five. The values compare to the previous question where I called the function separately for each observation are the same.
...
The graph shows the largest log-likelihood values for the probabilities of the candidate events around the MLE and the curve peaks. The curve around the MLE looks flatter compared to the other candidates, and the log likelihood value takes a smaller value the farther the x value is from the MLE.
...
$$
\log \left( p \left( H \mid AB, \mu \right) \right) \propto \log \binom{AB}{H}+H\log\mu + \left( AB - H \right) \log\left( 1 - \mu \right)
$$
$$ 
\log \left( p \left( H \mid AB, \mu \right) \right) \propto H\log\mu + \left( AB - H \right) \log\left( 1 - \mu \right)
$$
...
What do you think?  
The figure in 3c compare to my figure in 2f is the same.
...
Compared with the plot in 2f and 3c, the log-likelihood value of the probability in 3e is larger when x takes the same value, and the curve takes a value close to 0 when it peaks.
...
What do you think?  
AS for the uniform prior, no matter how we change the value of x, the result is 1. The image is shown as a horizontal line with a value of 1.
The event probability roughly under 5%  and above 50% values that are "ruled out" by the informative prior as the second plot shows.
As for the informative prior,the beta_pdf value peaks when mu is taken close to 0.25. The farther x is taken from 0.25, the smaller the y value and the flatter the curve.
...
$$
\log \left( p \left( m \mid N, \mu \right) \right) \propto m\log\mu+ \left(N - m\right)\log\left(1-\mu\right)
$$
...
Add the un-normalized log-likelihood to the un-normalized log-prior
$$ 
m\log\mu+ \left(N - m\right)\log\left(1-\mu\right)+\left(a-1\right)\log\mu + \left(b-1\right) \log\left(1-\mu\right)
$$
Then perform the required algebra to derive $a_{new}$ and $b_{new}$.  
$$
\left(m+a-1\right)log\mu+\left( N-m+b-1\right)log\left(1-\mu\right)
$$
...
$$
$$
b_{new}=b+\left(N-m\right)
$$
the expressions for the updated or posterior Beta shape parameters.
$$
\log \left( p \left( \mu \mid a_{new}, b_{new} \right) \right) \propto \left(a_{new}-1\right)\log\mu + \left(b_{new}-1\right) \log\left(1-\mu\right)
$$
### 5d)
...
The unknown event probability $\mu$:
$$ 
\log \left( p \left( \mu \mid a_{new}, b_{new} \right) \right) \propto \left(a_{new}-1\right)\log\mu + \left(b_{new}-1\right)\log\left(1-\mu\right)
$$
The first derivative of the un-normalized log-posterior:

$$
\frac{d}{d\mu} \left(\log \left( p \left( \mu \mid a_{new}, b_{new} \right) \right)\right) = \frac{d}{d\mu} \left(\left(a_{new}-1\right)\log\mu \right) + \frac{d}{d\mu}\left(\left(b_{new}-1\right)\log\left(1-\mu\right)\right)
$$
$$
\frac{d}{d\mu} \left(\log \left( p \left( \mu \mid a_{new}, b_{new} \right) \right)\right) = \left(a_{new}-1\right)\left( \frac{1}{\mu}\right) + \left(b_{new}-1\right)\left(\frac{ 1}{1-\mu}*(-1)\right)
$$
$$
\frac{d}{d\mu} \left(\log \left( p \left( \mu \mid a_{new}, b_{new} \right) \right)\right) = \frac{a_{new}-1}{\mu}- \frac{b_{new}-1}{1-\mu}
$$
...
Set the derivative from your solution to Problem 5d) equal to zero:
$$
\frac{d}{d\mu} \left(\log \left( p \left( \mu \mid a_{new}, b_{new} \right) \right)\right) = \frac{a_{new}-1}{\mu}- \frac{b_{new}-1}{1-\mu}=0
$$
$$
\frac{a_{new}-1}{\mu}= \frac{b_{new}-1}{1-\mu}
$$
$$
\left({1-\mu}\right)\left(a_{new}-1\right)=\mu\left(b_{new}-1\right)
$$
$$
a_{new}-\mu a_{new}-1+\mu=\mu b_{new}-\mu
$$
$$
\mu b_{new}+\mu a_{new}-2\mu=a_{new}-1
$$

$$
\mu\left(a_{new}+b_{new}-2 \right)=a_{new}-1
$$
$$ 
...
What do you think?  
First, let's compare the width under the curve, which is the uncertainty. We know that the sum of the area probabilities of each graphs under the curve is 1. Obviously, the informative prior curve is narrower, so the uncertainty is smaller.
In the previous graph showing the uniform prior, the peak is close to 5, while in the informative prior, the peak is over 7. So the second graph has a better 'the most probable value'.
...
What do you think?  
We have two kinds of a prior，One prior is uniform, while the other is "informative".
With a very small sample size in question 6, we can clearly see that the effect of different prior conditions on the confidence intervals shown in the figure is significant.
In the seventh question, after we increased the sample size, there is basically no difference between the graphs shown under the two different prior conditions, which proves that the influence of the prior probability on the posterior probability can be greatly reduced when the sample data size is large enough.
